...
"""
mutable struct CartPoleContinuousEnv{T <: AbstractFloat} <: AbstractMDP{Vector{T}, Vector{T}}
    const 𝑔::T
    const 𝑚₁::Float64
    const 𝑚ₚ::Float64
    const 𝑙::Float64
...
function in_absorbing_state(cp::CartPoleContinuousEnv)::Bool
    𝑥, _, θ, _ = cp.state
